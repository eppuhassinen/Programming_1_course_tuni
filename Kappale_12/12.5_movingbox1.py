...
class MovingBox:
    ...
    def __init__(self, box_name):
        """
        :param box_name: str, name of the box
        """
    ...
    def add_item(self, item_name, item_count):
        ...
        # Adds items if there is already items with same name in the dict
        if item_name in self.__box_items:
            self.__box_items[item_name] += item_count

        # Else makes the item and assigns item_count to it
        else:
            self.__box_items[item_name] = item_count

    def remove_item(self, item_name, item_count):
        """
        Removes items from the box.
        Returns None if the item is not in self.__box_items.
        Returns None if param item_count is larger than the amount of items
        in the dict
        If there remains 0 items gets the item removed from the dict
        :param item_name: str, name of item
        :param item_count: int, amount of items to remove
        :return: True, if no errors, else None
        """

        if item_name not in self.__box_items:
            return None

        if self.__box_items[item_name] < item_count:
            return None

        self.__box_items[item_name] -= item_count
        if self.__box_items[item_name] == 0:
            self.__box_items.pop(item_name)
        return True

    def list_content(self):
        """
        Prints all contents in self.__box_items in alphabetical order
        in format

        Box "self.__box_name" contains 'total items' items.
          'amount' 'item'
          'amount' 'item'
        :return: None
        """

        # calculates the amount of total items
        total_items = 0
        for item in self.__box_items:
            total_items += self.__box_items[item]

        # prints first row
        print(f"""Box "{self.__box_name}" contains {total_items} items. """)

        # prints box contents in alphabetical order
        for item_name in sorted(self.__box_items):
            print(f"{self.__box_items[item_name]:3d} {item_name}")

    def transfer_item(self, target_box, item_name, item_count):
        """
        Transfers items from self to target_box
        :param target_box: MovingBox, target of the transfer
        :param item_name: str, name of the item
        :param item_count: int, number of items to transfer
        :return: True if no errors, else None
        """

        # checks if the item is in self.__box_items
        if item_name not in self.__box_items:
            return None

        # checks if self has enough items
        if item_count > self.__box_items[item_name]:
            return None

        # removes items from self
        self.remove_item(item_name, item_count)

        # adds items to target_box
    ...
    def search_item(self, item_name):
        """
        Returns the number of item_name in the box
        :param item_name: str, item to search for
        :return: int, number of item_name in self.__box_items
        """
...
def convert_str_to_int(word):
    ...
    :param (str) word: a word supposedly presenting an integer value.
    :return: int | NoneType
    """
    try:
        result = int(word)
...
def remove_from_box(all_boxes, list_of_additional_info):
    ...
    *list_of_additional_info*, then an error message is displayed.
    If the box exists but doesn't contain the named item,
...
def transfer_from_box_to_another_box(all_boxes, list_of_additional_info):
    ...
    if not all_boxes[source_name].transfer_item(all_boxes[target_name],
